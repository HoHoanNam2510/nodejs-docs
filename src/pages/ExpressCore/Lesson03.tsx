...
export default function Lesson03({ isDone, onToggleDone }: Props) {
  return (
    ...
    >
      <Sec title="Khái niệm">
        Express cung cấp các method tương ứng HTTP verbs: <code>app.get</code>,{' '}
        <code>app.post</code>,<code>app.put</code>, <code>app.patch</code>, <code>app.delete</code>.
        Mỗi route handler nhận <code>(req: Request, res: Response)</code> — TypeScript type hai
        object này để IDE autocomplete và bắt lỗi. Với TypeScript, bạn có thể type chính xác{' '}
        <code>req.params</code>,<code>req.body</code>, <code>req.query</code> qua generics của{' '}
        <code>Request</code>.
      </Sec>
          ...
          rows={[
            {
              line: '1',
              explanation:
                'Request<Params, ResBody, ReqBody, Query> — 4 generics cho type safety. Thứ tự: Params, ResponseBody, RequestBody, QueryString.',
            },
            {
              line: '2',
              explanation:
                'Pick<ITodo, "title"> — utility type lấy chỉ field "title" từ ITodo. Dùng cho body POST: chỉ gửi title, server tự tạo id/createdAt.',
            },
            {
              line: '3',
              explanation:
                'res.status(201).json(...) — method chaining: set status code rồi gửi JSON. 201 Created dùng khi tạo resource mới thành công.',
            },
            {
              line: '4',
              explanation:
                'return res.status(404).json(...) — return để dừng function sau khi gửi response. Không return → code tiếp theo vẫn chạy, gây lỗi "headers already sent".',
            },
            {
              line: '5',
              explanation:
                'crypto.randomUUID() — built-in Node.js (v14.17+), tạo UUID v4. Không cần cài thêm package, type safe với string.',
            },
            {
              line: '6',
              explanation:
                'Object.assign(todo, req.body) — merge req.body vào object todo có sẵn. Chỉ update fields được gửi lên, giữ nguyên fields còn lại.',
            },
            {
              line: '7',
              explanation:
                'res.status(204).send() — 204 No Content: xóa thành công, không trả body. Dùng .send() thay .json() vì 204 không được có body.',
            },
          ]}
        />
      </Sec>
      ...
      <Sec title="Lỗi thường gặp">
        <Callout type="warn">
          Gọi <code>res.json()</code> hai lần → "Cannot set headers after they are sent to the
          client". Luôn <code>return</code> sau mỗi response hoặc dùng <code>if/else</code> tách
          biệt để đảm bảo chỉ một response được gửi.
        </Callout>
        <Callout type="note">
          Thứ tự routes quan trọng. <code>app.get('/todos/stats', ...)</code> phải đứng trước
          <code>app.get('/todos/:id', ...)</code> — route cụ thể trước route dynamic. Express match
          route theo thứ tự đăng ký.
        </Callout>
      </Sec>
