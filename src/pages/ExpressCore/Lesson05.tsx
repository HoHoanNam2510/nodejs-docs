...
export default function Lesson05({ isDone, onToggleDone }: Props) {
  return (
    ...
    >
      <Sec title="Khái niệm">
        <code>req.body</code> chứa data client gửi lên trong request body (POST/PUT/PATCH). Mặc định
        Express không parse body — phải bật middleware <code>express.json()</code> trước. TypeScript
        cho phép type chính xác shape của body qua 3rd generic của <code>Request</code>:
        <code>Request&lt;Params, ResBody, BodyType, Query&gt;</code>. Điều này giúp IDE
        autocomplete, bắt typo, và tăng bảo mật bằng cách chỉ accept đúng fields được khai báo.
      </Sec>
          ...
          rows={[
            {
              line: '1',
              explanation:
                'app.use(express.json()) — middleware parse Content-Type: application/json. Phải đứng TRƯỚC routes. Thiếu dòng này → req.body là undefined.',
            },
            {
              line: '2',
              explanation:
                'Request<{}, {}, CreateUserBody> — 3rd generic type req.body. {} là placeholder cho Params và ResBody không dùng.',
            },
            {
              line: '3',
              explanation:
                'age?: number — optional field trong interface. TypeScript type là number | undefined, không cần gửi lên nếu không có.',
            },
            {
              line: '4',
              explanation:
                'priority = "medium" — default parameter khi destructure. Nếu client không gửi priority, sẽ dùng "medium". Type inference tự hiểu.',
            },
            {
              line: '5',
              explanation:
                'Request<{ id: string }, {}, UpdateTodoBody> — combine Params và Body generics. id từ URL, updates từ body, đều được typed.',
            },
            {
              line: '6',
              explanation:
                'interface bảo mật body: chỉ fields trong interface mới được TypeScript accept. Client không thể inject field "role: admin" nếu không khai báo.',
            },
            {
              line: '7',
              explanation:
                'ids: string[] — array type trong body interface. req.body.ids là string[], đảm bảo bulk operations nhận đúng format.',
            },
          ]}
        />
      </Sec>
      ...
      <Sec title="Lỗi thường gặp">
        <Callout type="warn">
          TypeScript chỉ kiểm tra lúc compile — ở runtime, <code>req.body</code> vẫn có thể là bất
          kỳ thứ gì client gửi. Luôn cần validate runtime (bài sau với <code>zod</code>) để đảm bảo
          data thực sự đúng format trước khi dùng.
        </Callout>
        <Callout type="note">
          Đặt <code>express.json()</code> trước tất cả routes. Thứ tự middleware trong Express là
          sequential — route đứng trước <code>express.json()</code> sẽ không thấy parsed body,
