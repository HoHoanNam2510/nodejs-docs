...
- Anchor link → smooth scroll đến lesson
...
interface Props {
  children: string;   // raw code text (đã có syntax spans trong HTML string)
  html?: boolean;     // nếu true: dùng dangerouslySetInnerHTML cho syntax highlighting
}

export default function CodeBlock({ children, html }: Props) {
...
- [x] `src/components/TocBar.tsx`: sticky progress bar + anchor links
